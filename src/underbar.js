(function() {
  ...
  _.last = function(array, n) {
    ...
    return n === undefined ? array[last] : array.slice(n, array.length);
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    if (accumulator === undefined) {
      Array.isArray(collection) ? accumulator = collection[0] : accumulator = collection[Object.keys(collection)[0]];
  ...
  _.defaults = function(obj) {
    var additionalObjects = Array.from(arguments);

    // _.each(additionalObjects, function(object) {
    ...
    //   });
    // });
    for (let i = 1; i < additionalObjects.length; i++) {
      let currentObject = additionalObjects[i];
      for (var key in currentObject) {
